...
Matrix::Matrix(const string &dataPath){
	...
	while (getline(in,record)){
		...
		vector<double> row( ( istream_iterator<double>( is ) ),istream_iterator<double>());
		mat.push_back(row);
	}
...
Matrix operator*(const Matrix& a, const Matrix& b) { 
	...
	if(a.colSize()==b.rowSize()){
		...
		throw invalid_argument( "The Two Matrix Size not match . Can not do Matrix multiplication." );
	}
}
...
Matrix operator >> (const Matrix& a, const Matrix& b) { 
	if(isMatrixSameSize(a,b)){
		...
		throw invalid_argument( "The Two Matrix Size not equal . Can not do Hadamard Product." );
	}
}
...
Matrix operator << (const Matrix& a, const Matrix& b) { 
	if(a.colSize()==1&&b.rowSize()==1){
		...
		throw invalid_argument( "The Two Matrix Size not match . Can not do Table Product." );
	}
}
...
bool operator == (const Matrix& a, const Matrix& b) { 
	if(isMatrixSameSize(a,b)){
		...
		throw invalid_argument( "The Two Matrix Size not equal . Can not do Check IF Equal." );
	}
}
